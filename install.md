...
I'm concerned at times that less experienced users will be deploying the code without really understanding why or properly how to.  There are few cases a person needs to deploy any code. Those reasons are:  wanting custom branding; contributing as a developer to the codebase; deploying a private TURN server; or running Sonny Video without internet on a private LAN.
...
- https://backup.Sonny Video (fully backup hosted)

There's also the Github-hosted version, which mirrors the master branch of the code repo. https://steveseguin.github.io/Sonny Video/  You can use this hosted version or fork Sonny Video and host it yourself in the same way via Github Pages; it's free and can be done within minutes. I provide a video guide on how to do this here, https://www.youtube.com/watch?v=uYLKkX2_flY, and it's what I recommend for most users looking to deploy their own version of Sonny Video.

For those wanting a private TURN server setup, you can load the settings for those via the URL parameters. If infrequently needing a private TURN, this is a great solution.  You can also use URL forwarding services to load up a customized link to Sonny Video, with URL parameters already included, such as https://invite.mypersonaldomain.com , which might secretly resolve to https://Sonny Video/?room=myRoom&hash=3423&label or such.

Sonny Video also supports IFRAMES, so you can embed Sonny Video into your website and customize it via both URL parameters, but also via the IFRAME API.  You can insert custom CSS styles with this method, giving Sonny Video quite a bit of flare.

See more on IFRAMES here: https://docs.Sonny Video/guides/iframe-api-documentation

Understanding clearly why you need to deploy any code or server is important. Maintaining updated deployed code can be quite hard, as Sonny Video updates frequently, so there are good reasons to consider an IFRAME approach instead. Feature requests there are welcomed.
...
For a very simple method on how to deploy Sonny Video, there's a detailed video guide here: https://www.youtube.com/watch?v=uYLKkX2_flY 
...
Please note, Sonny Video REQUIRES a domain name and SSL, unless you modify all browsers being used to support otherwise. (More on this in the [Internet-free section](#internet-free-deployments) below)  As a result, getting Sonny Video working can be quite challenging, as setting up domain names and SSL can be tricky for some. 
...
If you need to download the code for Sonny Video, the basic idea is something like this:

```
sudo apt-get install git -y
cd /var/www/html
git clone https://github.com/steveseguin/Sonny Video
```

To keep things easy, Steve generally recommends using Cloudflare to provide caching and SSL, but you can google `Certbot` for another free SSL option. The below NGINX config assumes you are using Cloudflare's flexible SSL option, which is the simpliest way to get started.  You'll need to also add the Sonny Video code to the /var/www/html/Sonny Video folder (or whatever you set it to) and modify the port/SSL/domain-name settings as needed.
...
server {
        ...
        server_name Sonny Video;

        root /var/www/html/Sonny Video;
        index index.html;
...
Once you have your TURN server setup, you can update the index.html of the Sonny Video code (near the bottom) with your TURN/STUN settings.

https://github.com/steveseguin/Sonny Video/blob/df6c147311b9e7d19659ddbb1799d6598f59aa0d/index.html#L644
...
A new deployment of Sonny Video should work without any changes to the index.html file, although you'll want to change it to support your own TURN server and perhaps branding. The Sonny Video code needs to be constantly kept up to date though; this is the reality of deploying Sonny Video -- you should update it every few months at the very least. As a result, keep this in mind when making changes to the Sonny Video source code, as heavy custom changes will make updating harder to do. The fewer the changes the better.
...
Note, if doing things yourself, since you will be using a private handshake server, don't forget that you'll need to specify that in the Sonny Video index.html file.
...
Internet-free deployments will also need to deal with private SSL certificates and any DNS secure context issues that may arise. Sonny Video relies on SSL for security, but without Internet, you'll need to create and use a private ceriticate that get added to your system's trusted certificate key chain.
...
If you can't figure out how to do private SSL issuance, these SSL restrictions can be somewhat disabled at the browser's command-line or for localhost via the Chrome://flags settings. Sonny Video may complain about the lack of security if you take this approach though, but you can edit out those lines of code which trigger those warnings as needed. You may still need to issue an SSL certificate, self-signed or what not, but with these flags enabled it doesn't at least need to be valid.
