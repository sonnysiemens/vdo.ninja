# Sonny Video Remote Control API Documentation

## Overview

Sonny Video's Remote Control API allows programmatic control of Sonny Video sessions via HTTP or WebSocket connections. This powerful API enables integration with stream decks, custom applications, and automation tools for controlling cameras, microphones, layouts, and other features.

## Basic Setup

To enable the API on any Sonny Video instance, add the `&api` parameter with a unique API key:

```
https://Sonny Video/?api=YOUR_UNIQUE_API_KEY&webcam
```

This key must be kept private and will be used to authenticate API requests. The same key must be used when making API calls to control this specific Sonny Video instance.
...
Structure: `https://api.Sonny Video/{apiKey}/{action}/{target}/{value}`

Examples:
```
https://api.Sonny Video/YOUR_UNIQUE_API_KEY/mic/false       // Mute microphone
https://api.Sonny Video/YOUR_UNIQUE_API_KEY/camera/toggle   // Toggle camera
```
...
```javascript
const eventSource = new EventSource(`https://api.Sonny Video/sse/YOUR_UNIQUE_API_KEY`);
...
These commands affect the local Sonny Video instance that has the API key enabled.
...
fetch("https://api.Sonny Video/YOUR_API_KEY/mic/toggle")
...
- The API has full control over the Sonny Video instance it's connected to
- All connections are encrypted over SSL/TLS

## Troubleshooting

- Ensure the API key matches exactly between Sonny Video and your requests
...
- Interactive demo: [companion.Sonny Video](https://companion.Sonny Video)
...
3. Modify the server URL in your Sonny Video instances:
   ```javascript
